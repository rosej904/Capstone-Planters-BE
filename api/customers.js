...
customersRouter.post('/login', async (req, res, next) => {
  ...
  try {
    ...
    if (cust && await bcrypt.compare(password, cust.password)) {
      ...
      res.cookie('jwtCust', token, {path:'/',domain:'.netlify.app', httpOnly:'true', secure:'true', maxAge: 60 * 60 * 24 * 7})
          res.send({
              name: "LoginSuccess",
...
customersRouter.post('/logout', async (req, res, next) => {
  ...
  try {
    ...
    if (auth == null) {
      ...
      res.cookie('jwtCust', 'null', {path:'/',domain:'.netlify.app',httpOnly:'true', secure:'true',  expires: new Date(0)})
      res.sendStatus(204)
    }
